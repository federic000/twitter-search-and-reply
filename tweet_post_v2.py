...
def post_tweet_reply(twid, mediaid):
    ...
    #the text to be tweeted comes from an OPEN-AI API query 
    quote = callgptv3("write a sentence about love and summer")
...
#calls OPEN AI API to make a sentence, a poem, or anything else to tweet
def callgptv3(query):
    openai.api_key = "xxxxx...your key goes here or as pass it as ENV_VAR...xxxxxxx"
    response = openai.Completion.create(model="text-davinci-003", prompt=query, \
     ...
     introtext = "a gift for you: "
     cleanupresp = response["choices"][0]["text"].strip()
     fulltext = introtext + cleanupresp
     return fulltext
